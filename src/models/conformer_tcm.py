...
class MyConformer(nn.Module):
  ...
  def forward(self, x, device):  # ← 반드시 필요
        x = x + self.positional_emb[:, :x.size(1), :]
        x = torch.stack([torch.vstack((self.class_token, x[i])) for i in range(len(x))])  # [bs, 1+time, emb]
    ...
    def __init__(self, device):
        ...
        try:
            ...
            if checkpoint.get("args") is None:
                checkpoint["args"] = SimpleNamespace(task="translation")
