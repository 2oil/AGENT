...
class ECAPAModel(nn.Module):
	...
	def eval_network(self, eval_list, eval_path):
		...
		tuneThreshold, EER, fpr, fnr = tuneThresholdfromScore(scores, labels, [1, 0.1])
		EER_threshold = tuneThreshold[-1][0]  # 마지막 항목이 EER 지점 (fnr ≈ fpr)

		fnrs, fprs, thresholds = ComputeErrorRates(scores, labels)
		minDCF, minDCF_threshold = ComputeMinDcf(fnrs, fprs, thresholds, 0.05, 1, 1)

		return EER, minDCF, EER_threshold
